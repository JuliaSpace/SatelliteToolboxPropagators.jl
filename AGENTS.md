...
- This package provides analytical and SGP4/SDP4 orbit propagators for the SatelliteToolbox.jl ecosystem and supports Julia 1.10 or newer within Julia 1.x (`[compat] julia = "1.10, 1.11, 1.12"`). It depends on **SatelliteToolboxBase.jl** v2, whose `KeplerianElements{Tanomaly, Tepoch, T}` selects the stored anomaly, on **SatelliteToolboxSgp4.jl** v3, which is a local development checkout until it is registered, on **SatelliteToolboxOrbitDataMessages.jl** v0.2, and on **NanoDates.jl**, used to build the epoch of the OMM assembled by `Propagators.mean_elements` for SGP4. `TLE` comes from the re-export of **SatelliteToolboxSgp4.jl**; **SatelliteToolboxTle.jl** is not a direct dependency.
- `src/SatelliteToolboxPropagators.jl` is the module entrypoint. It loads, in this order, the generic `Propagators` API (`src/api/Propagators.jl`), the shared types (`src/types.jl`), the propagator-specific API methods (`src/api/*.jl`), the numerical kernels (`src/propagators/*.jl`), the `show` methods of the propagator structures (`src/show.jl`), and finally the precompile workloads (`src/precompile.jl`). Preserve this dependency order when adding code.
...
- Every mean elements output (`Propagators.mean_elements`, the fitting functions, and the epoch updates) returns `KeplerianElements{MeanAnomaly}` for every propagator, including SGP4, whereas the SGP4 fitting functions return an `OrbitMeanElementsMessage` by default or a `TLE` selected by the sink argument. The osculating propagators store their osculating elements as `KeplerianElements{TrueAnomaly}`.
- Every fitting function returns `(mean_elements, covariance, stats)`, where `stats` is a `NamedTuple` with `converged`, `iterations`, `position_rmse` [m], `velocity_rmse` [m / s], and `total_rmse`, as in **SatelliteToolboxSgp4.jl** v3. The SGP4 wrappers rescale the statistics from kilometers to SI units. The shared algorithm throws the exported `MeanElementsFitDivergenceError` when the iterations diverge and validates `max_iterations`; SGP4 throws `Sgp4FitDivergenceError`. The keyword `mean_elements_epoch` accepts a Julian Day or a `DateTime`, and the allocating fitting and epoch-update functions accept the constants with the same keyword of the initialization functions (`j2c`, `j4c`, `m0`, `sgp4c`).
- The keyword block of the fitting functions is documented once per propagator in `fit_*_mean_elements`; the mutating variants and the `Propagators.fit_mean_elements[!]` wrappers cross-reference it.
...
- The printed representations use the public tree helpers of **SatelliteToolboxBase.jl** v2.1 (`print_compact`, `print_tree`, `print_tree_body`, `print_status`, `epoch_string`, `format_value`, `type_name`, and the types `PrintedField` and `PrintedSection`), imported by name in `src/SatelliteToolboxPropagators.jl` and `src/api/Propagators.jl`. Every propagator structure overloads `SatelliteToolboxBase.print_tree_body` with its fields and sections, and the API wrappers print their header followed by the body of the structure returned by `Propagators.propagator_data`; `Propagators.is_initialized` keeps the compact form from touching undefined fields; it relies on the private helper `_last_instant` in `src/types.jl`, which returns the `NaN` set by the empty constructors of the propagators of mean elements until an initialization function replaces it, and reads the instant of the osculating propagators from the wrapped propagator (`J2OsculatingPropagator` and `J4OsculatingPropagator` have no `Δt` field). Both are optional API functions with defaults in `src/api/Propagators.jl`.

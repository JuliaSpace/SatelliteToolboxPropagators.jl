...
- The printed representations use the public helpers of **SatelliteToolboxBase.jl** v2.1 (`print_compact`, `print_elements`, `print_field`, `compact_string`), called with the module prefix. The API wrappers print their header and delegate to the rich representation of the structure returned by `Propagators.propagator_data`; `Propagators.is_initialized` keeps the printing from touching undefined fields. Both are optional API functions with defaults in `src/api/Propagators.jl`.

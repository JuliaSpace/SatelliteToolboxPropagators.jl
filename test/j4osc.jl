...
@testset "Fitting Mean Elements for the J4 Osculating Orbit Propagator" verbose = true begin
    ...
    j4oscd = j4osc_init(orb_input)
    ret    = map(t -> j4osc!(j4oscd, t), 0:10:12_000)
    vr_i   = first.(ret)
    vv_i   = last.(ret)
    vjd    = j4oscd.j4d.orb₀.t .+ (0:10:12_000) ./ 86400
    ...
    @testset "Errors" begin
        ...
        @test_throws ArgumentError fit_j4osc_mean_elements(vjd[1:end-1], vr_i, vv_i)
        @test_throws ArgumentError fit_j4osc_mean_elements(vjd, vr_i[1:end-1], vv_i)
        @test_throws ArgumentError fit_j4osc_mean_elements(vjd, vr_i, vv_i[1:end-1])

        # Wrong dimensions in the weight vector
        # ==================================================================================

        @test_throws ArgumentError fit_j4osc_mean_elements(
            vjd,
            vr_i,
            vv_i;

...
@testset "J2 Osculating Orbit Propagator" verbose = true begin
    ...
    @testset "Constructor" begin
        ...
        j2oscd = J2OsculatingPropagator{Float64, Float64}(j2d, orb)

        # Test some random fields.
        @test j2oscd.j2d == j2d
        @test j2oscd.j2d.Δt == 0
        @test j2d.orbk == orb

        orb = KeplerianElements(0.0, 8_000_000, 0, 0, 0, 0, 0)
        orbp = Propagators.init(Val(:J2osc), orb)
        @test orbp.j2oscd.j2d.orb₀ isa KeplerianElements{MeanAnomaly, Float64, Float64}
    end
...
@testset "Fitting Mean Elements for the J2 Osculating Orbit Propagator" verbose = true begin
    ...
    @testset "Constants Keyword" begin
        ...
        orb, P, stats = redirect_stdout(devnull) do
            Propagators.fit_mean_elements(Val(:J2osc), vjd, vr_i, vv_i; j2c = J2C_EGM2008_F32)
        end
        ...
        orb_alt, P_alt = fit_j2osc_mean_elements(vjd, vr_i, vv_i; j2c = J2C_JGM03, verbose = false)
        pd = j2osc_init(orb_input; j2c = J2C_JGM03)
        orb_ref, P_ref = fit_j2osc_mean_elements!(pd, vjd, vr_i, vv_i; verbose = false)
...
@testset "Update J2 Osculating Mean Elements Epoch" begin
    ...
    orb_alt   = update_j2osc_mean_elements_epoch(orb_input, new_epoch; j2c = J2C_JGM03)
    pd        = j2osc_init(orb_input; j2c = J2C_JGM03)

    @test orb_alt == update_j2osc_mean_elements_epoch!(pd, orb_input, new_epoch)
    @test orb_alt != update_j2osc_mean_elements_epoch(orb_input, new_epoch)

## Description #############################################################################
#
#   Tests for the ML-∂SGP4 Propagators API extension.
#
#   With a zero-correction model, ML-∂SGP4 must match plain SGP4.  We reuse the CBERS 2
#   expected results from the SGP4 tests to validate the Propagators API wrapper against
...
############################################################################################
#
# Using the same CBERS 2 TLE and expected results as sgp4.jl.  See that file for the full
...
@testset "ML-∂SGP4 orbit propagator" verbose = true begin
    ...
    @testset "General API Functions" begin
        orbp = Propagators.init(Val(:MLdSGP4), tle)

        @test orbp isa OrbitPropagatorMLdSGP4
        @test Propagators.name(orbp) == "ML-∂SGP4 Orbit Propagator"
        @test Propagators.epoch(orbp) ≈ jd₀
    end
    ...
    @testset "Float64" begin
        ...
        orbp = Propagators.init(Val(:MLdSGP4), tle; sgp4c = sgp4c_wgs72)
    ...
    @testset "Float32" begin
        ...
        orbp = Propagators.init(Val(:MLdSGP4), tle; sgp4c = sgp4c_wgs72_f32)

...
function j2osc!(
    ...
    orbk = KeplerianElements(
        j2d.orb₀.t + Tepoch(t) / 86400, a_osc_k, e_osc_k, i_osc_k, Ω_osc_k, ω_osc_k, f_osc_k
    )
...
function fit_j2osc_mean_elements!(
    ...
    if verbose
        println(
            "$(cy)ACTION:$(cd)   Fitting the mean elements for the J2 osculating propagator.",
        )
        @printf(
    ...
    j2oscd_ad =
        jacobian_method isa ForwardDiffJacobian ? _create_j2osc_ad_propagator(j2oscd) :
        nothing

    # The finite-difference Jacobian overwrites the propagator it uses. Giving it its own
    # propagator lets us initialize `j2oscd` once per iteration instead of once per
    # measurement.
    j2oscd_fd =
        jacobian_method isa FiniteDiffJacobian ? _create_j2osc_fd_propagator(j2oscd) :
        nothing
...
function _j2osc_jacobian(
    ...
    j2oscd_ad::Union{Nothing, J2OsculatingPropagator} = nothing,
    j2oscd_fd::Union{Nothing, J2OsculatingPropagator} = nothing,
) where {T <: Number, Tepoch <: Number}
    ...
    fd::J2OsculatingPropagator{Tepoch, T} = isnothing(j2oscd_fd) ? j2oscd : j2oscd_fd

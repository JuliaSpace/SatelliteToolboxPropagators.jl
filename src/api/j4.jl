...
The other keywords are the same as in [`fit_j4_mean_elements`](@ref).
...
"""
function Propagators.init!(orbp::OrbitPropagatorJ4, orb₀::KeplerianElements)
    j4_init!(orbp.j4d, orb₀)
    return nothing
end
...
Propagate the orbit of the J4 orbit propagator `orbp` to `Δt` [s] after the epoch of
...
"""
function Propagators.propagate!(orbp::OrbitPropagatorJ4, Δt::Number)
    return j4!(orbp.j4d, Δt)
end
...
function Base.copy(orbp::OrbitPropagatorJ4{Tepoch, T}) where {Tepoch <: Number, T <: Number}
    return OrbitPropagatorJ4{Tepoch, T}(copy(orbp.j4d))
end
